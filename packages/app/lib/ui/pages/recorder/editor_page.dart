...
class _EditorPageState extends State<EditorPage> with SingleTickerProviderStateMixin {
      ...
      if (_files.isNotEmpty) {
          try {
              for (var path in _files) {
                  ...
                  _fileSplits[path] = [
                      TrackSplit(number: 1, start: Duration.zero, end: null, title: 'Track 1')
                  ];
              }
  ...
  Future<void> _generateWaveform(String wavPath) async {
    ...
    final waveformPath = path.join(dir, '${path.basenameWithoutExtension(wavPath)}_waveform.png');
    ...
    if (!await file.exists()) {
        try {
            await Process.run('ffmpeg', [
                '-y',
                '-i', wavPath,
                '-filter_complex', 'showwavespic=s=1024x120:colors=green',
                '-frames:v', '1',
                waveformPath
            ]);
        } catch (e) {
  ...
  Future<void> _process(Release metadata, bool treatSidesAsDiscs) async {
      ...
      try {
          ...
          if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Processing Complete!')));
            ...
            builder: (context, setState) => AlertDialog(
              ...
              content: SingleChildScrollView(
                  child: Column(
                      ...
                      children: [
                          TextField(controller: artistController, decoration: const InputDecoration(labelText: 'Artist', hintText: 'Required')),
                          TextField(controller: albumController, decoration: const InputDecoration(labelText: 'Album', hintText: 'Required')),
                          ...
                          if (_files.length > 1)
                              CheckboxListTile(
                                  title: const Text('Treat each recording as a separate disc'),
                          ...
                          Row(
                              children: [
                                  Expanded(
                                      child: Text(coverPath != null ? path.basename(coverPath!) : 'No Cover Selected', 
                  ...
                  FilledButton(
                      onPressed: () {
                          if (artistController.text.isEmpty || albumController.text.isEmpty) {
                              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Artist and Album are required')));
                              return;
                          }
                          ...
                          Navigator.pop(context, {'release': release, 'treatSidesAsDiscs': treatSidesAsDiscs});
                      }, 
        ...
        appBar: AppBar(
            ...
            bottom: _files.length > 1 
                ? TabBar(
                    ...
                    tabs: _files.asMap().entries.map((e) => Tab(text: 'Side ${String.fromCharCode(65 + e.key)}')).toList(),
                  )
                : null,
        ),
        body: Column(
                    children: [
                            ...
                            child: Column(
                                children: [
                                    Row(
                                        ...
                                        children: [
                                            Text(_formatDuration(_currentPosition), style: const TextStyle(color: Colors.white, fontFamily: 'monospace')),
                                            Text(_formatDuration(_totalDuration), style: const TextStyle(color: Colors.grey, fontFamily: 'monospace')),
                                        ],
                                    ),
                                    Expanded(
                                        child: LayoutBuilder(
                                    ...
                                    Row(
                                        ...
                                        children: [
                                             IconButton(
                                                ...
                                                onPressed: () => _player.seek(_currentPosition - const Duration(seconds: 5))
                                             ),
                                             StreamBuilder<bool>(
                        ...
                        Expanded(
                            child: splits != null ? ListView.builder(
                                itemCount: splits.length,
                                itemBuilder: (context, index) {
                                    final track = splits[index];
                                    final isSelected = (_currentPosition >= track.start) && 
                                                        ...
                                                        children: [
                                                            Expanded(
                                                                child: OutlinedButton(
                                                                    ...
                                                                    child: Text('Start: ${_formatDuration(track.start)}'),
                                                                ),
                                                            ),
                                                            ...
                                                            Expanded(
                                                                child: OutlinedButton(
                                                                    ...
                                                                    child: Text(track.end == null ? 'End: (File End)' : 'End: ${_formatDuration(track.end!)}'),
                                                                ),
                                                            ),
                                                        ],
                                                    )
                        ...
                        Padding(
                            ...
                            child: SizedBox(
                                ...
                                child: FilledButton.icon(
                                    ...
                                    label: Text(processor.isProcessing ? 'Processing (Splitting WAV to FLAC)...' : 'Confirm Regions & Process'),
